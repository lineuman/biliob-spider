...
output_file = 'D:/DataSource/B站/总计粉丝增加-2020-02-10.csv'
# field = 'cArchive_view'
...
mid_list = []
for each_author in db['author'].find({field: {'$gt': 2000000}}, {'mid': 1}).batch_size(200):
...
def add_data(mid):
  ...
  while (current_date < min(end_date.timestamp(), x[-1])):
    ...
    if begin_date >= x[0] and current_date < x[-1] and current_date > x[0]:
      ...
      title = db['video'].find_one({'mid': mid, 'datetime': {
          '$lt': datetime.datetime.fromtimestamp(current_date)}}, {'title': 1}, sort=[('datetime', -1)])
      if title != None and 'title' in title:
        title = title['title']

      d[c_date].append((each_author['name'], fans_func[1],
                        each_author['face'], title))
...
with open(output_file, 'w', encoding="utf-8-sig") as f:
  f.writelines('"date","name","value","title"\n')
  for each_date in d:
    for each_data in d[each_date]:
      f.writelines('"{}","{}","{}","{}"\n'.format(each_date, each_data[0],
                                                  each_data[1], each_data[3]))
